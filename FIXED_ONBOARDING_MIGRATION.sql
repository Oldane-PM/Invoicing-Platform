-- =====================================================
-- Employee Onboarding System - FIXED VERSION
-- =====================================================
...
DROP POLICY IF EXISTS "Employees can view own onboarding status" ON employees;
DROP POLICY IF EXISTS "Employees can update own onboarding info" ON employees;
DROP POLICY IF EXISTS "Admins can manage all onboarding" ON employees;

-- Allow employees to read their own onboarding status
CREATE POLICY "Employees can view own onboarding status"
  ON employees
  FOR SELECT
  USING (id = auth.uid() OR id::text = current_setting('app.employee_id', true));

-- Allow employees to update their own onboarding fields
CREATE POLICY "Employees can update own onboarding info"
  ON employees
  FOR UPDATE
  USING (id = auth.uid() OR id::text = current_setting('app.employee_id', true))
  WITH CHECK (
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifications') THEN
    -- Add notification type enum values if they don't exist
    BEGIN
