-- =====================================================
-- Separate Onboarding System from Employees Table
-- =====================================================
-- This migration refactors onboarding workflow into dedicated
-- tables, keeping employees as a clean "system of record" for
-- active staff only. Includes audit trail, encryption support,
-- and secure RLS policies.
-- =====================================================

-- =====================================================
-- 1. Create Enums for Onboarding State Machine
-- =====================================================
...
-- =====================================================
-- 2. Create onboarding_cases table (main workflow)
-- =====================================================
...
COMMENT ON COLUMN onboarding_cases.current_state IS 'Current state in the onboarding workflow';
COMMENT ON COLUMN onboarding_cases.rejection_reason IS 'Admin explanation if rejected';

-- =====================================================
-- 3. Create onboarding_personal table (1-to-1)
-- =====================================================
...
-- =====================================================
-- 4. Create onboarding_banking table (1-to-1, encrypted)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS onboarding_banking (
  ...
  -- For now storing as text, but production should use pgcrypto
...
COMMENT ON COLUMN onboarding_banking.account_number_encrypted IS 'Encrypted account number - use pgcrypto in production';

-- =====================================================
-- 5. Create onboarding_contract table (admin-managed)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS onboarding_contract (
  ...
  -- Compensation
  rate NUMERIC(10, 2),
  rate_type TEXT, -- 'hourly', 'monthly', 'annual'
  currency TEXT DEFAULT 'JMD',
  ...
  -- Document storage (if using Supabase Storage)
...
COMMENT ON COLUMN onboarding_contract.contract_file_path IS 'Path to signed contract in storage';

-- =====================================================
-- 6. Create onboarding_events table (audit trail)
-- =====================================================
...
COMMENT ON COLUMN onboarding_events.payload IS 'Flexible JSON for event context (e.g., changed fields, rejection reason)';
...
-- =====================================================
-- 8. Enable RLS on all onboarding tables
-- =====================================================
...
-- =====================================================
-- 10. RLS Policies for onboarding_cases
-- =====================================================
...
-- =====================================================
-- 11. RLS Policies for onboarding_personal
-- =====================================================

-- Employees can view/update their own personal info
...
-- =====================================================
-- 12. RLS Policies for onboarding_banking
-- =====================================================

-- Employees can view/update their own banking info
...
-- =====================================================
-- 13. RLS Policies for onboarding_contract
-- =====================================================
...
-- =====================================================
-- 14. RLS Policies for onboarding_events
-- =====================================================
...
-- Admins can view/create all events
CREATE POLICY "Admins can manage all onboarding events"
  ON onboarding_events FOR ALL
  USING (is_admin());

-- =====================================================
-- 15. Create approve_onboarding function (admin only)
-- =====================================================
...
-- =====================================================
-- 16. Create reject_onboarding function (admin only)
-- =====================================================
...
-- =====================================================
-- 17. Gate timesheet submissions (RLS on submissions)
-- =====================================================
...
-- =====================================================
-- 18. Create view for admin onboarding queue
-- =====================================================
...
SELECT 
  ...
  op.state_parish,
  op.country,
  op.personal_completed_at,
  
  -- Banking info (limited fields for security)
  ob.bank_name,
  ob.account_type,
  ob.currency,
  ob.banking_completed_at,
...
-- 
-- Next steps:
-- 1. Update signup API to create onboarding_cases row
-- 2. Update DAL queries to use new tables
-- 3. Update UI to fetch from onboarding_cases/personal/banking
